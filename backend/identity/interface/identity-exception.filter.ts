...
import type { IdentityError } from '#identity/application/errors.js';
...
export class IdentityExceptionFilter implements ExceptionFilter {
  catch(err: IdentityError | HttpException, host: ArgumentsHost): void {
    const res = host.switchToHttp().getResponse<Response>();

    // Guards (SessionAuthGuard, RolesGuard) throw Nest's own HttpExceptions
    // (401/403) directly rather than a coded IdentityError — pass those
